...
// into a travel/stays 글 frontmatter as image/imageAlt/imageCredit/imageSource.
//
// 2026-09-04에 이식됐던 spots/stays 버전(heroImage: 로컬 파일 다운로드 + astro:assets)을
// main 리라이트 이후의 현재 스키마(travel/stays, image: 원격 URL 문자열 4종 세트)에
// 맞게 다시 옮긴 버전. 검증 로직(제목 점수, 제외 키워드, 콘텐츠타입 필터)은 그대로
// 재사용했다 — 검색 1순위 결과를 그대로 쓰지 않고, 제목이 기대한 장소명과 맞는지,
// 부속시설(상품관·매표소·주차장 등)이 아닌지 확인한 뒤 통과한 후보의 사진만 쓴다.
// 통과 후보가 없으면 실패로 끝내고 호출자(fill-hero-images.mjs)가 Pexels 로 폴백한다.
//
// 로컬 파일을 받지 않고 원격 이미지 URL을 그대로 frontmatter에 적는다는 점이
// daytrevel/이전 spots 버전과의 가장 큰 차이다 — 현재 스키마의 image 필드가
// astro:assets 이 아니라 <img src> 로 바로 쓰이는 문자열이기 때문.
//
// Usage:
//   node --env-file=.env scripts/set-tour-hero-images.mjs              # KEYWORDS 전체
...
// slug -> TourAPI 검색 설정 (실제 장소명). 여기 없는 슬러그는 frontmatter의
// keyword 필드를 검색어로 자동 사용한다(아래 defaultSpecFor 참고) — areaCode 등으로
// 후보를 좁히고 싶을 때만 이 표에 추가하면 된다.
//   문자열      : 검색어 하나
...
const KEYWORDS = {
	...
	'ochang-rest-area': { keywords: ['오창휴게소'], areaCode: AREA.충북, match: '오창휴게소' },
	'muju-hyangnosan-recreation-forest': { keywords: ['향로산자연휴양림', '무주 향로산자연휴양림', '향로산 자연휴양림'], areaCode: AREA.전북, match: '향로산' },
	'incheon-metropolitan-museum': { keywords: ['인천시립박물관', '인천광역시립박물관'], match: '박물관' },
	'wolgot-fishing-port': { keywords: ['월곶포구', '시흥 월곶포구'], match: '월곶' },
	'wondong-station-plum-village': { keywords: ['원동역', '순매원'], areaCode: AREA.경남, match: '원동' },
	'jirisan-jungsanri-course': { keywords: ['지리산 천왕봉', '중산리탐방지원센터', '지리산 중산리'], areaCode: AREA.경남, match: '지리산' },
	'descendants-of-the-sun-filming-site': { keywords: ['삼탄아트마인'], areaCode: AREA.강원, match: '삼탄아트마인' },
	'yunjerim-forest-garden': { keywords: ['윤제림', '보성 윤제림'], match: '윤제림' },
	'jwagusan-recreation-forest': { keywords: ['좌구산자연휴양림', '좌구산천문대'], areaCode: AREA.충북, match: '좌구산' },
	'chunhyang-rest-area': { keywords: ['춘향휴게소'], match: '춘향휴게소' },
	'hwanggeumsan-elephant-rock': { keywords: ['황금산', '황금산 코끼리바위'], areaCode: AREA.충남, match: '황금산' },
	'yangpyeong-cheongchun-museum': { keywords: ['추억의 청춘뮤지엄', '청춘뮤지엄'], areaCode: AREA.경기, match: '청춘' },
	'suseongdong-valley': { keywords: ['인왕산 수성동 계곡', '수성동계곡', '인왕산 수성동'], areaCode: AREA.서울, match: '수성동' },
	'hanam-union-park': { keywords: ['하남유니온파크', '유니온타워'], areaCode: AREA.경기, match: '유니온' },
	'yeongcheon-rest-area': { keywords: ['영천휴게소'], match: '영천휴게소' },
	'seocheon-seafood-market': { keywords: ['서천수산물특화시장'], areaCode: AREA.충남, match: '수산물특화시장' },
...
// image/imageAlt/imageCredit/imageSource 4종을 세트로 넣거나 갈아치운다
// (AGENTS.md: "넣으려면 4개를 모두 넣는다"). season이 있으면 그 뒤, 없으면
// country 뒤에 새로 삽입한다 — 기존 예시들의 frontmatter 필드 순서와 동일하다.
...
// contentid로 공식 소개 페이지를 하나 찾아본다. homepage 필드가 있으면 그 안의
// 첫 링크를, 없으면 대한민국 구석구석의 표준 상세 페이지 URL로 대신한다.
async function sourcePageOf(spot) {
	try {
		const [detail] = await callApi('detailCommon2', {
		...
		const href = detail?.homepage?.match(/href="([^"]+)"/)?.[1];
		if (href) return href;
	} catch {
		// homepage 조회 실패는 폴백으로 넘어간다
	}
	return `https://korean.visitkorea.or.kr/detail/ms_detail.do?cotid=${spot.contentid}`;
}
...
// KEYWORDS에 없는 슬러그는 글 자체의 keyword frontmatter를 검색어로 쓴다.
async function defaultSpecFor(slug) {
	const filePath = await findContentFile(slug);
	const text = await readFile(filePath, 'utf-8');
	const m = text.match(/^keyword:\s*'((?:[^'\\]|\\.)*)'/m) ?? text.match(/^keyword:\s*"([^"]*)"/m);
	if (!m) return null;
	return { keywords: [m[1]] };
}
...
async function main() {
	...
	for (const slug of slugs) {
		try {
			const raw = argSlug ? argKeyword ?? KEYWORDS[slug] : KEYWORDS[slug];
			const spec = raw ? toSearchSpec(raw) : await defaultSpecFor(slug);
			if (!spec) throw new Error(`no keyword for slug ${slug}`);
			...
			await patchPost(contentFile, {
				...
				imageAlt: `${found.spot.title} 전경`,
				imageCredit: CREDIT,
				imageSource: await sourcePageOf(found.spot),
			});

			console.log(`  saved (${found.spot.title} / ${found.spot.addr1 ?? ''})`);
			ok++;
		} catch (err) {
	...
	console.log(`TourAPI 사진에는 "${CREDIT}" 출처 표기가 imageCredit에 들어간다.`);

...
// and wire it into a travel/stays 글 frontmatter as image/imageAlt/imageCredit/imageSource.
//
...
// slug -> 검색 설정. 여기 없는 슬러그는 글의 keyword frontmatter로 검색한다.
...
// 사진 한 장 단위의 공식 상세 페이지 URL 규칙이 공개돼 있지 않아, 출처 링크는
// 갤러리 최상위 페이지로 대신한다. 더 정확한 딥링크를 알게 되면 이 상수만 고치면 된다.
const GALLERY_HOME = 'https://gallery.visitkorea.or.kr/';
...
async function main() {
	...
	for (const slug of slugs) {
		try {
			...
			await patchPost(contentFile, {
				...
				imageAlt: found.item.galTitle,
				imageCredit: CREDIT,
				imageSource: GALLERY_HOME,
			});

			console.log(`  saved (${found.item.galTitle} / 촬영: ${found.item.galPhotographer ?? '정보 없음'})`);
			ok++;
		} catch (err) {
	...
	console.log(`관광사진갤러리 사진에도 "${CREDIT}" 출처 표기가 imageCredit에 들어간다.`);

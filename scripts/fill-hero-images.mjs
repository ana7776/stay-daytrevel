#!/usr/bin/env node
// Fill missing hero images for travel/stays posts.
//
// 세 단계로 소스를 시도한다:
...
//
// Usage:
//   node --env-file=.env scripts/fill-hero-images.mjs            # image 없는 글 전체
//   node --env-file=.env scripts/fill-hero-images.mjs a,b,c      # 이 슬러그들만
//   node scripts/fill-hero-images.mjs --list                     # 대상만 출력
//
// Env: TOUR_API_KEY (KorService2), TOUR_PHOTO_API_KEY (PhotoGalleryService1), PEXELS_API_KEY
//      FORCE=true      이미 image 가 있어도 다시 받기
...
async function hasImage(slug) {
	return /^image:/m.test((await findPost(slug)).text);
}
...
const targets = args.length
	...
	: (await Promise.all((await allSlugs()).map(async (s) => ((await hasImage(s)) ? null : s)))).filter(Boolean);

if (!targets.length) {
	console.log('모든 글에 이미지가 이미 있습니다.');
	process.exit(0);
}
...
for (const slug of targets) {
	...
	const before = process.env.FORCE === 'true' ? null : await hasImage(slug);
	if (before === true) {
		console.log('  이미 image 가 있어 건너뜁니다 (force 로 덮어쓰기 가능)');
		continue;
	}

	// 성공 판정은 자식 스크립트의 종료 코드로만 한다.
	// image 존재 여부를 성공 신호로 쓰면, force 로 다시 받을 때
	// 이전 실행이 남긴 image 때문에 실패가 성공으로 잡힌다.
